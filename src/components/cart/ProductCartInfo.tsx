...
const ProductCartInfo = ({ product }: { product: EnrichedProducts }) => {
  ...
  const handleAddItem = useCallback(async () => {
    await addItem(category, productId, size, variantId, price);
    router.refresh();
  }, [category, productId, size, variantId, price, router]);

  const handleDelItem = useCallback(async () => {
    await delOneItem(productId, size, variantId);
    router.refresh();
  }, [productId, size, variantId, router]);
